 ...
 *  produce an warning when run through the Intel High Level Synthesis    *
 *  compiler. The API and the implementation remains the same as defined  *
 *  and implemented by Mentor Graphics for the AC Datatypes 3.7 release.  *
 *                                                                        *
 *************************************************************************/
...
#warning This version of ac_int.h is direct from Mentor Graphics and will not produce efficient hardware on the Intel High Level Synthesis Compiler. Please use HLS/ac_int.h for an efficient implementation.
#endif

// When compile for FPGA, we need to to rule out std::cout in component. Case:563860
#ifdef HLS_SYNTHESIS
...
#pragma builtin
  ...
  inline void iv_const_shift_l(const int *op1, int *r) {
    ...
    if(!B) {
      ...
      iv_extend<(const int)ishift>(r, 0);
      ...
      if(s31) {
        ...
        if(Nr > M1) {
          ...
          iv_extend<(const int)(Nr-M1-1)>(r+M1+1, r[M1] < 0 ? ~0 : 0);
        }
      } else {
        for(unsigned i=ishift; i < M1 ; i++)
          r[i] = op1[i-ishift];
...
namespace ac {
  ...
  struct int_range {
    static constexpr bool l_s = LowerBound < 0, u_s = UpperBound < 0;
    static constexpr bool signedness = l_s || u_s;
    static constexpr int l_nbits = nbits<AC_ABS(LowerBound+l_s)+l_s>::val;
    static constexpr int u_nbits = nbits<AC_ABS(UpperBound+u_s)+u_s>::val;
    static constexpr int nbits = AC_MAX(l_nbits, u_nbits + (!u_s && signedness));
